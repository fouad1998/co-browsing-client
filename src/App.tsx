...
import { coBrowsingReducer, coBrowsingReducerInterface, CO_BROWSING_REDUCER, TOOL_USING } from './coBrowsingReducer';
...
const App: React.FC<{}> = (props) => {
  ...
  const [state, dispatch] = React.useReducer(coBrowsingReducer, {
  ...
  const URLBarClickHandler = React.useCallback((event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
  ...
  const URLEmptyClickHandler = React.useCallback((event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    event.stopPropagation()
    dispatch({
      type: CO_BROWSING_REDUCER.URL,
      payload: { currentURL: "" }
  ...
  const showToolsHandler = React.useCallback((event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    event.stopPropagation()
    dispatch({
  ...
  const selectToolHandler = React.useCallback((tool: TOOL_USING) => {
    dispatch({
  ...
  const settingsShowHandler = React.useCallback((event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    event.stopPropagation()
    dispatch({
    ...
    <Layout style={{ height: "100vh", maxHeight: "100vh", maxWidth: "100vw", minHeight: "100vh", minWidth: "100vw" }}>
      <Row style={{ overflow: 'hidden' }}>
        ...
        <Col span={24} style={{ height: "60px", padding: "0 20px", background: "#f5f5f5" }}>
          <Row style={{ height: "inherit", alignItems: "center" }}>
            <Col span={12} style={{ display: "flex", alignItems: "center" }}>
              <h1 style={{ padding: 0, margin: 0, fontSize: "x-large", fontVariant: "all-petite-caps", fontStyle: "oblique", color: "#3535d2", fontWeight: "bolder", letterSpacing: "0.5mm" }}>Emplorium</h1>
            </Col>
            <Col span={12}>
              <ToolsStyle>
                {state.showTools && (<>
                  {[[<NearMeRoundedIcon />, TOOL_USING.MOUSE], [<BrushIcon />, TOOL_USING.BRUSH], [<CreateIcon />, TOOL_USING.PENCIL], [<Crop32Icon />, TOOL_USING.RECTANGLE]].map((e, index) => {
                    return (<Button key={index} shape='circle' type={state.selectedTool === e[1] ? "primary" : "default"} onClick={event => {
                      event.stopPropagation()
        ...
        <Col span={24} style={{ height: 50, padding: '0 10px', background: "#EEE" }}>
          <Row style={{ justifyContent: "center", alignItems: "center", height: "inherit" }}>
            ...
            <Col span={18} offset={1}
              style={{ height: "65%", fontSize: "15px", borderRadius: 5 }} >
              <Row style={{ height: "100%" }}>
                <Col span={24} style={{ height: "inherit" }}>
                  <Row style={{ height: "inherit" }}>
                    <Col span={modifyURL ? 24 : 22} onClick={URLBarClickHandler}
                      style={{ padding: modifyURL ? 0 : "0px 5px 0 10px", height: "inherit", display: "flex", alignItems: "center", whiteSpace: "nowrap", background: "white", overflow: "hidden" }}>
                      {!modifyURL && state.currentURL}
                      {modifyURL && <Input autoFocus value={state.currentURL} onChange={URLOnChangeHandler} onBlur={URLBarBlurHandler}
                        addonAfter={<Button type="ghost" onClick={URLEmptyClickHandler}> <HighlightOffIcon /></Button>}
        ...
        <Modal visible={state.showSettings} title="Settings" onOk={settingsSaveHandler} onCancel={settingsCancelHandler} keyboard={true}>
          <Row>
            <Col span={24}>
              <Row>
                <Col span={20}>
                  Same user screen size
                </Col>
                <Col span={4}>
                  <Switch checkedChildren="On" unCheckedChildren="Off" defaultChecked={state.sameUserScreenSize} onChange={sameUserScreenSizeHandler} checked={state.sameUserScreenSize} />
                </Col>
              </Row>
            </Col>
          </Row>
        ...
        <Col span={24} style={{ height: "calc(100vh - 110px)", background: "red" }}>
          <div ref={ref as React.RefObject<HTMLDivElement>} style={{ height: '100%', width: '100%', position: "relative" }}></div>        </Col>
      </Row>
    </Layout >
  );
}
